...
from YesNoTrainer import check_hazard, HazardSatelliteResNet


class harzardRemover:
    def __init__(self):
        self.model = HazardSatelliteResNet()
    ...
    def check_hazard_path(self, path):
        return check_hazard(path, self.model,device,0.3)


h = harzardRemover()


filenames = [f for f in os.listdir("Utils/images") if f.endswith('.jpeg')]
rejected_spots = 0
i = 0
print(len(filenames))
import os
